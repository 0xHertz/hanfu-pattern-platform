...
export const QUJU_SIZES: GarmentSize[] = [
...
export interface QujuDimensions {
  ...
  wrapExtension: number
}

export function calculateQuju(
  height: number,
  bust: number,
  ease: number = 120
): QujuDimensions {
  ...
  return {
    garmentLength: Math.round(height * 0.85),
    garmentBust,
    hemWidth: Math.round(garmentBust * 0.62),
    cuffWidth: Math.round(bust * 0.30),
    armSpan: Math.round(height * 1.10),
    collarWidth: Math.round(bust * 0.19),
    collarEdgeWidth: 65,
    wrapExtension: Math.round(garmentBust * 0.70),
  }
}
