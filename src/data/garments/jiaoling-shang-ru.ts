...
export const JIAOLING_SHANGRU_SIZES: GarmentSize[] = [
...
export interface JiaolingShangRuDimensions {
...
export function calculateJiaolingShangRu(
  height: number,
  bust: number,
  ease: number = 100
): JiaolingShangRuDimensions {
  const garmentBust = bust + ease
  return {
    garmentLength: Math.round(height * 0.374),
    garmentBust,
